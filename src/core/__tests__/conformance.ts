// Copied from @azrtydxb/core/dist/__tests__/adapter-conformance.js
// (that file exists in dist but is not in the package's exports map, so we
// vendor it here to avoid deep-import restrictions.)
import { describe, it, expect, beforeEach } from "vitest";
import type { SqliteAdapter } from "@azrtydxb/core";
...
export function runConformanceSuite(
  ...
  describe(`SqliteAdapter conformance: ${name}`, () => {
    ...
    it("get returns the inserted row", () => {
      ...
      const row = db.get("SELECT * FROM items WHERE id = ?", ["a"]);
    ...
    it("all returns rows in insertion order", () => {
      ...
      const rows = db.all("SELECT id FROM items ORDER BY id");
